...
$$
\begin{align*}
f(\boldsymbol y) &= \frac1{\sqrt{(2 \pi)^2 \cdot \det(\boldsymbol\Sigma)}} \exp \left( -\frac12 (\boldsymbol y - \boldsymbol\mu)^T \boldsymbol\Sigma^{-1} (\boldsymbol y - \boldsymbol\mu) \right)\\
...
print(paste("The mean of the OOB error is", round(mean(oobErr), 5), "while the maen of the test error is", round(mean(testErr), 5)))
```
